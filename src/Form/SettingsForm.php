...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    try {
      ...
      $ssl_available = in_array('ssl', stream_get_transports(), TRUE) && !defined('ACQUIA_CONNECTOR_TEST_DEVELOPMENT_NOSSL') && $config->get('spi.ssl_verify');
    ...
    if (!empty($subscription)) {
      $form['subscription'] = [
        '#markup' => $this->t('Subscription: @sub <a href=":url">change</a>', ['@sub' => $subscription, ':url' => $this->url('acquia_connector.setup')]),
      ];
    }
    ...
    if ($this->moduleHandler->moduleExists('help')) {
      $help_url = \Drupal::url('help.page', ['name' => 'acquia_connector']);
    }
    else {
    ...
    if (!empty($identifier) && !empty($key)) {
      $ssl_available = (in_array('ssl', stream_get_transports(), TRUE) && !defined('ACQUIA_CONNECTOR_TEST_DEVELOPMENT_NOSSL'));
      ...
      $form['connection']['spi']['module_diff_data'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('Source code'),
        '#default_value' => (int) $config->get('spi.module_diff_data') && $ssl_available,
        '#description' => $this->t('Source code analysis requires a SSL connection and for your site to be publicly accessible. <a href=":url">Learn more</a>.', [':url' => $help_url]),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($values['machine_name'] != $this->config('acquia_connector.settings')->get('spi.site_machine_name')) {
      ...
      $response = \Drupal::service('acquia_connector.spi')->sendFullSpi(ACQUIA_CONNECTOR_SPI_METHOD_CREDS);
