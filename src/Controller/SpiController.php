   ...
   */
    public function sendFullSpi($method = '') 
    {
        ...
        if ($this->checkEnvironmentChange()) {
            ...
            drupal_set_message(
                t(
                    'SPI data not sent, site environment change detected. Please <a href="@environment_change">indicate how you wish to proceed</a>.', array(
            ...
            foreach ($expected_data_types as $key => $values) {
                if (!array_key_exists($key, $response_data) || gettype($response_data[$key]) != $expected_data_types[$key]) {
                    \Drupal::logger('acquia spi')
                      ->error(
                          'Received SPI data definition does not match expected pattern while checking "@key". Received and expected data: @data', array(
                          '@key' => $key,
